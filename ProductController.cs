...
namespace WebBanHang.Areas.Admin.Controllers
{
    ...
    [HttpGet]
    public ActionResult Trash(string currenFilter, string SearchString, int? page)
        {
            var listProduct = new List<Product>();
            if (SearchString != null)
            {
            ...
            {
                listProduct = objWebsiteBanHangEntities.Products.Where(n=>n.Id==null).ToList();
            }
            ViewBag.CurenFilter = SearchString;
            int pageSize = 7;
            int pageNumber = (page ?? 1);
            listProduct = listProduct.OrderByDescending(n => n.Id==null).ToList();
            return View(listProduct.ToPagedList(pageNumber, pageSize));
        }
